...
async def scan_ports(host: str, ports: List[int], concurrency: int, show_progress: bool = True) -> Dict[int, Optional[str]]:
    ...
    if HAS_TQDM and show_progress:
        for coro in tqdm(asyncio.as_completed(tasks), total=len(tasks), desc='Scanning ports'):
            try:
                # This part is complex to map back to port, so we will gather all at the end
                await coro
            except Exception:
                pass
...
async def run_scan_flow(args) -> Dict[str, Any]:
    ...
    host_for_scan = ips[0]
    print(f"[+] Target resolved to: {host_for_scan}")
    ...
    print(f"[+] Starting layer-1 quick scan on {host_for_scan} (ports: {len(ports_to_scan)})")
    open_ports = {}
    ...
    if args.autolayer2:
        if not args.full:
            ...
            # Exclude ports already scanned
            deep_ports_to_scan = [p for p in deep_ports if p not in ports_to_scan]
            
            deep_scan_res = await scan_ports(host_for_scan, deep_ports_to_scan, args.concurrency, show_progress=not args.quiet)
    ...
    if args.nmap:
        ...
        nmap_res = run_nmap_scan(host_for_scan, ports=','.join(str(p) for p in report.get('open_ports', [])[:200]))
